...
const targetDetail = {
  // 제어반 예시
  '1-1': {
    detail: '처리장치 전체의 상황을 모니터링하고 조절하는 부분으로\n기본적으로 장치에 유입, 배출되는 유량과 전해조에 인가되고\n있는 전압, 전류를 확인할 수 있다.\n\n또한 센서를 추가설치하고 IoT 시스템을 이용하여 유입수/처리수의 온도, pH, 전기전도도 및 현장 영상 등을 확인할 수 있다.\n\nIoT 시스템은 제어반과 현장의 정보를 당사 홈페이지를 통해\n실시간으로 확인할 수 있도록 개발되었다. NE-Series는\nIoT 시스템을 통하여 빠르게 현장상황을 모니터링하고 기민하게 피드백을 제공하여 돌발상황에 효율적으로 대처할 수 있다.',
    src: '/images/제어반.png',
    src2: '/images/제어반2.jpg',
  },
  '1-2': {
    detail: '처리장치로 유입되는 공정수 유량을 측정하여 전체공정을\n안정적으로 운전하는 역할을 수행한다. 처리수의 성상에 따라\n특수한 경우 디지털 유량계가 사용 될 수도 있다.\n\n일반적으로 공정수의 현재 유입량을 측정하기 위해 보통 처리장치 전단에 순간유량계가 설치되며, 전체 처리량을 측정하기 위해서는 시스템 후단에 적산유량계가 설치된다.\n\n본 시뮬레이션에서 유량계는 입자성 오염물질이 제거되고\n용존성 오염물질이 함유된 공정수의 전해조 유입량을 조절한다.',
    src: '/images/유량계.png',
  },
  '1-3': {
    detail: '교류전기를 직류전기로 변환하는 장치이다. 전해조가 안정적인 전기화학적 반응을 일으키기 위해 필요한 일정한 직류 전기를 공급하는 역할을 한다.\n\n정류기로 안정적으로 직류 전기를 제어해야만, 전극에 부하를 최소화하면서 최적조건으로 전기화학적 반응을 이끌어낼 수 있다.  따라서 수처리 시스템을 장시간 연속운전하기 위해서는, 일정한 출력을 보장하는 정류기가 필수적이다.\n\n전해조의 구성, 처리대상 원수의 성상에 따라서 최적 전류/전압 범위가 바뀌며, 최적 전류/전압의 범위에 따라서 사용되는 정류기 역시 달라질 수 있다.',
    src: '/images/정류기.png',
  },
  '2-1': {
    detail: '전해조란 전기 에너지를 이용해 전기화학반응을 일으키는 장치이다. 전해조를 통과하면서 공정수의 용존성 오염물질들이 전기화학적 반응을 통해 제거되어 최종 처리수로 배출된다.\n\n이온, 분자 형태로 존재하여 응집,여과로 제거가 불가능한 용존성 오염물질을 전기화학적 반응을 통해 효과적으로 처리할 수 있다. 전해조는 사용목적에 따라 전기화학적 산화반응이나 환원반응을 일으키도록 설계할 수 있다.\n\nNE Series의 전해조는 산화반응을 이용한 직접산화(0차 반응)와 간접산화(1,2차 반응)를 일으켜 강한 산화물질을 만들어내며, 이를 통해 수중 총유기탄소와 암모니아성 질소 등의 오염물질을\n분해/제거한다.\n\n당사의 전해조는 모듈형 설계로 전극개수와 처리용량 조절이\n용이하고 생산성을 극대화하였으며, 폐쇄형 구조를 채택하여\n배가스로 인한 부스바 부식문제에 대응하였다.',
    src: '/images/img-전해조.png',
  },
  '2-2': {
    detail: '처리장치로 공정수나 폐수를 유입시키는 역할을 수행한다.\n\n처리대상 원수의 성상과 처리량에 따라 다양한 종류와 용량의 펌프를 사용할 수 있다.\n\n펌프의 종류를 결정하는 요소로는 원수의 pH, 온도, 부유물질(Suspended Solid 등) 함유 여부, 필요한 처리량, 오염물질의 성질 등이 있으며, 설계상 필요한 경우 펌프 수량과 위치를 최적위치로 조정할 수 있다.\n\n본 시뮬레이션에서 펌프는 입자성 오염물질이 제거되고 용존성 오염물질이 함유된 공정수를 다음 단계로 이송한다.',
    src: '/images/펌프.png',
  },
  '3-1': {
    detail: '폐수에 녹지 않고 입자형태로 떠다니는 오염물질들을 제거하는 장치이다. 대개 화학약품을 통해 작은 입자들을 큰 입자로 응집한 후에 침전 혹은 부상하여 필터 등으로 여과하는 방식을 사용한다.\n\nNE-Series에서는 입자성 오염물질 처리장치를 통과하면서 공정수의 입자성 오염물질들이 분리되어, 용존성 오염물질이 남은 공정수는 다음 전기화학적 처리단계로 이송된다.',
    detail2: '입자성 오염물질 처리장치에는 침전, 용존공기부상(DAF),\n여과장치 등이 있다.',
    src: '/images/img-입자성 오염물질 처리장치.png',
    src2: '/images/입자성 처리장치 추가사진.png',
  },
  '3-2': {
    detail: '고객의 수요와 현장상황, 적용되는 법령에 따라 요구되는 처리수준이 달라지기 때문에 효율적인 처리결과는 현장에 따라서 상이할 수 있다.\n\n고객이 요구하는 수준이나 법적 기준치 이하로 처리된 물은 배출되어 공정수로 이용되거나 방류된다.\n\n본 시뮬레이션에서는 전해조에서 용존성 오염물질까지 제거된 최종 처리수가 배출되는 모습을 표현하고 있다.',
    src: '/images/처리수.png',
  },
};
                ...
                <>
                  ...
                  {targetDetail[selectedHotspot.level2Id].src && (
                    <img
                      ...
                      style={{ width: '100%', height: 'auto', marginTop: '12px', borderRadius: '12px', }}
                    />
                  )}
